...
const Index = ({ navigation }: StackScreenProps<MainNavigationParams, 'MainScreen'>) => {
  ...
  const minCountElems = useMemo(
    () => Math.floor(Metrics.screenHeight / Metrics.getHeight(60)),
    []
  );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tableContainer}>
        {isLoading ? <View style={{alignItems: 'center'}}><Text style={{fontSize: Metrics.getHeight(60)}}>LOADING</Text></View> : driversData?.MRData.DriverTable.Drivers.map((driver) => (
          <TouchableOpacity style={styles.tableElement} onPress={() => onPressDriver(driver.driverId)} key={driver.driverId as Key}>
      ...
      <View style={styles.paginationContainer}>
        {[1,
          ...(maxCountPage > 6 ?
            page >= 5 ?
              page >= maxCountPage - 3 ? [-2,maxCountPage - 4, maxCountPage - 3,maxCountPage - 2,maxCountPage - 1,maxCountPage] :
              [-2, page - 1,page, page + 1, -1, maxCountPage] :
              [2,3,4,5, -1, maxCountPage] :
            [2,3,4,5])].map((item) => (
          <PagintaionItem pageNumber={item} selected={page === item} key={item} onPress={() => onPressPaginationElement(item)}/>)
        )}
